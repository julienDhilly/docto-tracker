...
// stop the script when a slot found. TODO: Check if slot alert as already been send and keep crawling
let find = false;
...
const c = new Crawler({
    ...
    callback: (error, res, done) => {
        if(error){
           ...
           try {
            const allowed = config.get('allowedCities').find(str => result.search_result.url.includes(`/${str}/`));
            if (!allowed) {
            ...
            if (result.total > 0) {
                find = true;
                console.log(result);
                console.log(result.search_result.id, 'YEAH', result.search_result.url);
...
function addQueues(ids) {
   c.queue(ids.map(id => {
      return `https://www.doctolib.fr/search_results/${id}.json?ref_visit_motive_ids%5B%5D=6970&ref_visit_motive_ids%5B%5D=7005&speciality_id=5494&search_result_format=json&force_max_limit=2`;
   }));
}